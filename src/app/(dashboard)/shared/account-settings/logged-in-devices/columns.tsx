...
const chipsColors = {
  ...
  // User role colors
  ADMIN: "bg-blue-lighter text-blue-dark",
  SUPER_ADMIN: "bg-red-lighter text-red-dark",
  MANAGER: "bg-green-lighter text-green-dark",
  STAFF: "bg-gray-lighter text-gray-dark",
} as { [key: string]: string };
        ...
        {row.original.teams.map((item: any, index: number) => {
          return (
            <Text
              ...
              as="span"
              className={cn(
                "inline-flex items-center justify-center gap-2 rounded-full px-2.5 py-1 text-xs font-semibold",
                chipsColors[item] || "bg-gray-lighter text-gray-dark"
              )}
            >
              {item}
            </Text>
